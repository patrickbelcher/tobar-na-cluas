...
imageWraps.forEach(wrap => {
    wrap.addEventListener('click', async () => {
        ...
        if (playerState.activeImageEl === wrap) {
...
function setActiveImage(wrap, isPlaying) {
    ...
    if (playerState.activeImageEl && playerState.activeImageEl !== wrap) {
        playerState.activeImageEl.classList.remove("active-mix");

        const prevIcon =
            playerState.activeImageEl.querySelector('.overlay-play-icon');

        if (prevIcon) prevIcon.innerHTML = playCircleSVG;
    }

    playerState.activeImageEl = wrap;
...
audio.addEventListener('pause', () => {
    playerIcon.innerHTML = playCircleSVG;
    updateActiveImageUI(false);
    persistPlayerState();
    clearInterval(saveInterval);
});

function updateActiveImageUI(isPlaying) {
    const el = playerState.activeImageEl;
    if (!el) return;

    const icon = el.querySelector('.overlay-play-icon');
    if (icon) icon.innerHTML = isPlaying ? pauseCircleSVG : playCircleSVG;

    if (isPlaying) el.classList.add("active-mix");
    else el.classList.remove("active-mix");
}
...
formatBtn.addEventListener("click", async () => {
    if (!playerState.activeImageEl) return;
    if (!seekingAllowed) return;

    const mixId = playerState.activeImageEl.dataset.id;
    const mix = window.mixes[mixId];
    if (!mix) return;
...
downloadBtn.addEventListener("click", () => {
    if (!playerState.activeImageEl) return;
    const id = playerState.activeImageEl.dataset.id;
    const mix = window.mixes[id]
